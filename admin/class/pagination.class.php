...
class pagination
{
	...
	public function calculate_pages($total_rows, $rows_per_page, $page_num, $show = 5)
	{
		$arr = array();
		// calculate last page
		...
		if( $page_num < 1 || !$page_num || !is_numeric($page_num) )
		{
		   $page_num = 1;
		} 
		...
		$arr['pages'] = $this->get_surrounding_pages($page_num, $last_page, $arr['next'], $show);
		return $arr;
	}
	function get_surrounding_pages($page_num, $last_page, $next, $show)
	{
		$arr = array();
...
/* $pagination2	    = $p->get_surrounding_pages($page, $pagination1['last'], $pagination1['next']);
$p = new pagination();
